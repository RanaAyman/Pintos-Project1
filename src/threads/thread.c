...
thread_hold_lock(struct lock *lock)
{
  ...
  list_insert_ordered(&cur->holding_locks, &lock->elem, lock_cmp_priority, NULL);
...
thread_update_priority(struct thread *t)
{
  ...
  if (!list_empty(&t->holding_locks))
  {
	...
	lock_pri = list_entry(list_front(&t->holding_locks), struct lock, elem)->max_priority;
    if (max_pri < lock_pri)
	  max_pri = lock_pri;
  }
  t->priority = max_pri;
...
thread_set_priority (int new_priority) 
{
  ...
  if (list_empty(&cur->holding_locks) || new_priority > old_priority)
  {
